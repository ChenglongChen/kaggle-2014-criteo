...
void save_model(Model const &model, std::string const &path)
{
    ...
    fwrite(&model.nr_factor2, sizeof(size_t), 1, f);
...
Model load_model(std::string const &path)
{
    ...
    size_t nr_factor, nr_factor2, nr_feature;
    fread(&nr_feature, sizeof(size_t), 1, f);
    fread(&nr_factor, sizeof(size_t), 1, f);
    fread(&nr_factor2, sizeof(size_t), 1, f);

    Model model(nr_feature, nr_factor, nr_factor2);
...
#pragma omp parallel for schedule(static) reduction(+:loss)
    for(size_t i = 0; i < spmat.Y.size(); ++i)
    {
        ...
        float const t = wTx(spmat, model, i)+wTx2(spmat, model, i);

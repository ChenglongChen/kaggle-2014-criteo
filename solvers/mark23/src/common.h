...
struct Model
{
    Model(size_t const nr_feature, size_t const nr_factor) 
        : W(nr_feature*kNR_FIELD*nr_factor*kW_NODE_SIZE, 0), 
          WA(nr_feature*kNR_FIELD*nr_factor*kW_NODE_SIZE, 0), 
...
inline float wTx2(SpMat const &spmat, Model &model, size_t const i, 
    float const kappa=0, float eta=0, float lambda=0, 
    bool const do_update=false)
{
    eta *= 0.1f;
    size_t const nr_factor = model.nr_factor;
    ...
    for(size_t idx1 = spmat.P[i]; idx1 < spmat.P[i+1]; ++idx1)
    {
        ...
        for(size_t f2 = 0; f2 < kNR_FIELD; ++f2)
        {
            if(f1 == f2)
                continue;
            ...
            float const * w1 = 
                model.W.data()+j1*kNR_FIELD*nr_factor*kW_NODE_SIZE+f2*nr_factor*kW_NODE_SIZE;
            float * wa1 = 
                model.WA.data()+j1*kNR_FIELD*nr_factor*kW_NODE_SIZE+f2*nr_factor*kW_NODE_SIZE;

            if(do_update)
            {
                float * wag1 = wa1 + nr_factor; 
                for(size_t d = 0; d < nr_factor; ++d, ++w1, ++wa1, ++wag1)
                {
                    float const g1 = lambda*(*wa1) + kappa*(*w1);

                    *wag1 += g1*g1;

                    *wa1 -= eta*qrsqrt(*wag1)*g1;
                }
            }
            else
            {
                for(size_t d = 0; d < nr_factor; ++d, ++w1, ++wa1)
                    t += (*wa1)*(*w1);
            }
        }
    }

...
SpMat read_data(std::string const path, size_t const reserved_size)
{
    ...
    while(fgets(line, kMaxLineSize, f) != nullptr)
    {
        ...
        while(1)
        {
            ...
            spmat.X.emplace_back(idx-1, val);
        }
...
void save_model(Model const &model, std::string const &path)
{
    ...
    fwrite(model.W.data(), sizeof(WNode), model.nr_feature, f);
    fclose(f);
}
...
Model load_model(std::string const &path)
{
    ...
    Model model(nr_feature);
    fread(model.W.data(), sizeof(WNode), model.nr_feature, f);
    fclose(f);
    return model;
}

...
double ctr_error(std::vector<double> const &pv, std::vector<int> const &tv) {
  double p_click = 0;
  for (auto p : pv) 
    p_click += 1/(1+exp(-p));
  int t_click = 0;
  for (auto t : tv) 
    if (t == 1) 
      t_click += t;
  if (t_click == 0) {
    printf("Warning: t_click = 0");
    return -1;
  } else {
    double const error = (p_click-static_cast<double>(t_click))/static_cast<double>(t_click);
    printf("CTR error = %.4f\n", error);
    return error;
  }
}
...
double calc_log_loss(const dvec_t& dec_values, const ivec_t& ty)
{
  double loss = 0;
  for(size_t i = 0; i < ty.size(); ++i) 
  {
    double const prob = lr(dec_values[i]);
    double const loss1 = (ty[i]==1)? log(prob) : log(1-prob);
    if(isinf(loss1))
        continue;
    loss += loss1;
  }
  loss = -loss/static_cast<double>(ty.size());
  printf("log loss = %.4f\n", loss);
  return loss;
}
...
//	validation_function(dec_values, true_labels);
	...
	ctr_error(dec_values, true_labels);
	calc_log_loss(dec_values, true_labels);

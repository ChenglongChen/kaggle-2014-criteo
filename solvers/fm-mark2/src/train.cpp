...
struct Option
{
    Option() : eta(0.1f), lambda(0.00002f), iter(15), nr_factor(4), nr_factor_real(4), nr_threads(1), save_model(true) {}
...
void train(SpMat const &Tr, SpMat const &Va, Model &model, Option const &opt)
{
    ...
    #pragma omp parallel for schedule(static)
            for(size_t i_ = 0; i_ < order.size(); ++i_)
            {
                ...
                wTx(Tr, model, i, kappa, opt.eta, opt.lambda, true, d);
            }

            printf("%3ld %3ld %8.2f %10.5f", iter, d, timer.toc(), 

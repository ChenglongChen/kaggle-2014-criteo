...
float logistic_func(float const t)
{
    return static_cast<float>(1/(1+exp(-t)));
}

float logistic_func_dt(float const t)
{
...
{
    float (*calc_prob) (float const t) = &logistic_func;
    float (*calc_prob_dt) (float const t) = &logistic_func_dt;
...
#pragma omp parallel for schedule(static)
        for(size_t i_ = 0; i_ < order.size(); ++i_)
        {
            ...
            float const prob = calc_prob(t);

            Tr_loss -= y*log(prob) + (1-y)*log(1-prob);
               
            float const kappa = -(y*(1/prob)+(y-1)*(1/(1-prob)))*calc_prob_dt(t);

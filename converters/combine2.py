...
def gen_hashed_fm_feats(feats, nr_bins):
    ...
    for field, feat in feats:
        if feat.startswith('I'):
            hashed_feats.append(str(hashstr(feat, nr_bins))+':'+'1') 
        else:
            if feat.endswith('-'):
                hashed_feats.append(str(hashstr(feat, nr_bins))+':'+'0') 
                hashed_feats.append(str(hashstr(feat, nr_bins)+1)+':'+'1') 
            else:
                hashed_feats.append(str(hashstr(feat, nr_bins))+':'+'1') 
                hashed_feats.append(str(hashstr(feat, nr_bins)+1)+':'+'0') 
    return hashed_feats

...
for size in ["x", "1", "0"]:
    ...
    cmd = 'converters/parallelizer.py -s 24 converters/num3.py tr.r{size}.csv tr.r{size}.num'.format(size=size)
    run(cmd) 
    cmd = 'converters/parallelizer.py -s 24 converters/num3.py va.r{size}.csv va.r{size}.num'.format(size=size)
    run(cmd) 

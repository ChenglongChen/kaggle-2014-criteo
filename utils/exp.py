...
for size in ["x", "100", "10"]:
    ...
    for data_csv, data_svm in [(tr_csv, tr_svm), (va_csv, va_svm)]:
        cmd = 'converters/parallelizer.py -n 16 converters/defender.py {data_csv} {data_svm}'\
            .format(data_csv=data_csv, data_svm=data_svm)
        subprocess.call(cmd, shell=True)

    cmd = './fm-sse-train -s 16 -t 30 -v {va_svm} {tr_svm} {model}'.format(va_svm=va_svm, tr_svm=tr_svm, model=model) 

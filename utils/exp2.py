...
for size in ["x", "1"]:
    ...
    cmd = 'converters/parallelizer.py -n 24 converters/num2.py tr.r{size}.csv tr.r{size}.num'.format(size=size)
    run(cmd) 
    cmd = 'converters/parallelizer.py -n 24 converters/num2.py va.r{size}.csv va.r{size}.num'.format(size=size)
    run(cmd) 

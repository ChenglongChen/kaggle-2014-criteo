...
for size in ["100"]:
    ...
    cmd = 'converters/parallelizer2.py -s 24 converters/combine2.py tr.r{size}.csv tr.r{size}.num.out tr.r{size}.fm'.format(size=size)
    run(cmd) 
    cmd = 'converters/parallelizer2.py -s 24 converters/combine2.py va.r{size}.csv va.r{size}.num.out va.r{size}.fm'.format(size=size)
    run(cmd) 

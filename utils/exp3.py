...
for size in ["100", "10"]:
    ...
    for data in ['tr', 'va']:
        cmd = 'converters/parallelizer.py -n 24 converters/gbdt.py {data}.r{size}.svm.num.gbdt {data}.r{size}.fm2'.format(size=size, data=data)
        subprocess.call(cmd, shell=True)

    cmd = './mark31-train -q -s 24 -t 20 -v va.r{size}.fm -v1 va.r{size}.fm2 tr.r{size}.fm tr.r{size}.fm2'.format(size=size) 
    subprocess.call(cmd, shell=True) 
